...
class CoRoutingEnv(gym.Env):
    ...
    def action_masks(self) -> np.ndarray:
        return np.array(
            [[True] * self._node_num + [False] * (133 - self._node_num)] * 2
        )
...
class GCNFeatureExtractor(BaseFeaturesExtractor):
    def __init__(
        ...
        self.gcn1 = GCNConv(self.node_space[1], 64)
    ...
    def forward(self, observations) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            # 빈 그래프 처리
            if filtered_nodes.size(0) == 0:
                # 빈 그래프인 경우 더미 노드 하나 생성
                filtered_nodes = torch.zeros((1, nodes.size(1)), device=device)

            if filtered_edge_index.size(1) == 0:
                # 빈 그래프인 경우 더미 엣지 하나 생성 (자기 자신과 연결)
                filtered_edge_index = torch.zeros(
...
# %%
def make_env():
    return Monitor(gym.make(ENV_ID, start_level=1))
...
def objective(trial: optuna.Trial) -> float:
    ...
    # Create the RL model.
    model = MaskablePPO(
        **kwargs, env=env, tensorboard_log="./optuna_sb3_co/", verbose=1, device="cuda"
    )
    ...
    try:
        ...
        model.learn(
            ...
            callback=[
                eval_callback,
                CurriculumCallback(verbose=1, success_threshold=0.9),
            ],
        )
        model.save(f"./optuna_sb3_co/saves/rl_model_{trial.number}")
        print("Learning end")
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./optuna_sb3_co/study.db"):
        study = optuna.create_study(
            study_name="sb3_co",
            sampler=sampler,
            pruner=pruner,
            direction="maximize",
            storage="sqlite:///./optuna_sb3_co/study.db",
        )
    else:
        print("Loading existing study...")
        study = optuna.load_study(
            study_name="sb3_co", storage="sqlite:///./optuna_sb3_co/study.db"
        )
    try:
