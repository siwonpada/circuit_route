...
class CoRoutingEnv(gym.Env):
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "edge_attributes": gym.spaces.Box(
                    low=0, high=self._candidate_num, shape=(300,), dtype=np.float32
                ),
            }
        )
    ...
    def action_masks(self) -> np.ndarray:
        ...
        if candidate_num < self._candidate_num:
            return np.array(
                [True] * candidate_num
                + [False] * (self._candidate_num - candidate_num),
            )
        return np.array([True] * self._candidate_num)
    ...
    def _unpack_state(self) -> dict:
        ...
        edge_attributes = np.zeros((300,), dtype=np.int32)
        for i, edge in enumerate(self._swap_candidate):
            edge_index = np.where(
                (self._edge_links[:, 0] == edge[0])
                & (self._edge_links[:, 1] == edge[1])
                | (self._edge_links[:, 0] == edge[1])
                & (self._edge_links[:, 1] == edge[0])
            )[0]
            for i in edge_index:
