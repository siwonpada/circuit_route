...
class SABRERoutingEnv(gym.Env):
    def __init__(
        ...
        self._random_layout = random_layout

        self.observation_space = gym.spaces.Box(
            low=-2, high=2, shape=(self._S_a, self._H), dtype=np.int32
        )

        self.action_space = gym.spaces.Discrete(self._S_a)
        return

    def action_masks(self) -> np.ndarray:
        candidate_num = len(self._swap_candidate)
        if candidate_num < self._S_a:
            return np.array(
                [True] * candidate_num + [False] * (self._S_a - candidate_num),
            )
        return np.array([True] * self._S_a)
    ...
    def step(
        ...
        return (
            self._unpack_state(isTerminated),
            self.reward(
    ...
    def _init_sabre(self, circuit: QuantumCircuit) -> None:
        ...
        if self._random_layout:
            data = self._layout.get_virtual_bits()
            values = list(data.values())
            keys = list(data.keys())
            random.shuffle(values)
            self._layout = Layout(dict(zip(keys, values)))
    ...
    def _update_front_layer(self) -> tuple[int, bool]:
        ...
        for node in self._front_layer:
            ...
            for nq in self._coupling_map.neighbors(q1):
                candidate.append(frozenset([q1, nq]))
            for nq in self._coupling_map.neighbors(q2):
                candidate.append(frozenset([q2, nq]))
        self._swap_candidate = list(map(tuple, list(set(candidate))))
    ...
    def _apply_swap(self, action: int) -> bool:
        ...
        if action >= len(self._swap_candidate):
            return True
        swap_action = self._swap_candidate[action]
    ...
    def _unpack_state(self, isTerminated: bool) -> np.ndarray:
        ...
        for i, (a, b) in enumerate(self._swap_candidate):
            ...
            temp_layout_map = temp_layout.get_virtual_bits()
            layers = self._sabre_dag.layers()
            for j, layer in enumerate(layers):
                ...
                new_distance = [
                    self._coupling_map.distance(
                        temp_layout_map[node.qargs[0]], temp_layout_map[node.qargs[1]]
                    )
                    for node in layer["graph"].op_nodes()
                ]
                result_matrix[i, j] = np.sum(
                    np.array(new_distance) - np.array(current_distances[j]),
                    dtype=np.int32,
                )
...
gym.register(id=ENV_ID, entry_point=SABRERoutingEnv)  # type: ignore
...
# %%
EX_NAME = "ibm_implementation_random_layout"
...
def objective(trial: optuna.Trial) -> float:
    ...
    model = MaskablePPO(
        **kwargs,
        env=env,
        tensorboard_log=f"./{EX_NAME}/",
        device="cuda",
    )
    # Create env used for evaluation.
    eval_env = Monitor(gym.make(ENV_ID, max_episode_steps=1000))
